...
//ajout film 
var filmAdd = document.getElementById("nom-film");
var btnEnvoyer = document.getElementById("btn-envoyer");

btnEnvoyer.addEventListener("click", getInfoFilm(filmAdd));

function getInfoFilm(filmAdd){
    console.log("coucou");
    let xhr = new XMLHttpRequest();
    let api = "https://api.themoviedb.org/3/search/movie?api_key=369494a80cb8a4b4357c06142e1b61cf&language=FR&query="+filmAdd;
    xhr.open("GET", api, true);
    xhr.send();
    xhr.onreadystatechange = function() {
        if(xhr.readyState == 4 && (xhr.status == 200 || xhr.status ==0)){
